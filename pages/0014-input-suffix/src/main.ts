...
class InputSuffixMaker {
  ...
  private getWidthOfInput() {
    ...
    (
      ['padding', 'font-size', 'font-family', 'letter-spacing'] as const
    ).forEach((styleName) => {
      (tmp.style as any)[styleName] =
        inputStyles[styleName as keyof CSSStyleDeclaration];
    });
